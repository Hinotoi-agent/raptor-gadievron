...
class WebScanner:
    ...
    def scan(self) -> Dict[str, Any]:
        ...
        all_findings.extend(self._phase_injection(crawl_data))
    ...
    def _phase_injection(self, crawl_data: Dict) -> List[WebFinding]:
        logger.info("Phase 6: Injection and fuzzing")
        if not self.llm or not self.fuzzer:
            logger.info("Phase 6: Skipped -- no LLM available")
            return []
        fuzzer = self.fuzzer
        ...
        # URL parameters
        for param in crawl_data.get("discovered_parameters", [])[:20]:
            for raw in fuzzer.fuzz_parameter(
                self.base_url, param, vulnerability_types=vuln_types
            ):
                _record(raw.get("url", self.base_url), param, raw)
        ...
        for form in crawl_data.get("discovered_forms", [])[:10]:
            ...
            for field_name, field_info in form.get("inputs", {}).items():
                if field_info.get("type") in ("hidden", "submit", "button"):
                    continue
                for raw in fuzzer.fuzz_parameter(
                    endpoint, field_name, vulnerability_types=["sqli", "xss"]
                ):
    ...
    def _phase_understand(self, crawl_data: dict, discovery: DiscoveryResult) -> Optional[dict]:
        """Optional Phase 6a: run /understand --map to build adversarial context.

        Feeds crawl surface into the understand pipeline. The resulting
        context-map.json is consumed by _phase_injection to prioritise
        high-value targets for fuzzing.
        """
        try:
            from core.orchestration.agentic_passes import run_understand_prepass
            from core.security.rule_of_two import is_interactive
        except ImportError:
            logger.info("Phase 6a: /understand integration unavailable")
            return None

        if not is_interactive():
            logger.info("Phase 6a: /understand skipped -- non-interactive mode (Rule of Two)")
            return None

        logger.info("Phase 6a: Running /understand to map attack surface")
        try:
            import shutil
            from pathlib import Path
            claude_bin = shutil.which("claude")
            if not claude_bin:
                logger.info("Phase 6a: claude not on PATH -- skipping /understand")
                return None
            ...
            if result.ran:
                logger.info("Phase 6a: /understand complete")
                self._phases_completed.append("understand")
                # Load the context map for injection phase prioritisation
                ctx_map_path = self.out_dir / "context-map.json"
                if ctx_map_path.exists():
                    from core.json import load_json
                    return load_json(ctx_map_path)
            else:
                logger.info(f"Phase 6a: /understand skipped: {result.skipped_reason}")
        except Exception as e:
            logger.debug(f"Phase 6a: /understand failed: {e}")
        return None
    ...
    def _phase_validate(self, findings: List[WebFinding]) -> List[WebFinding]:
        ...
        try:
            ...
            findings_for_validate = [f.to_dict() for f in needs_review]
            findings_input = self.out_dir / "web_findings_for_validation.json"
            save_json(findings_input, {"findings": findings_for_validate})
